		...
		OnClickListener 
{
	...
	public void onClick(View v) 
	{
		...
		{
			if((GIEditLayersKeeper.Instance().getState() != GIEditingStatus.WAITING_FOR_OBJECT_NEWLOCATION)&&(m_btnNew.isChecked()))
			{
				if(!GIEditLayersKeeper.Instance().CreateNewObject())
				{
					return;
				}
				GIEditLayersKeeper.Instance().setState(GIEditingStatus.WAITING_FOR_OBJECT_NEWLOCATION);
			...
			{
				GIEditLayersKeeper.Instance().setState(GIEditingStatus.RUNNING);
		...
		{
			if(GIEditLayersKeeper.Instance().getState() != GIEditingStatus.WAITIN_FOR_SELECT_OBJECT)
			{
				GIEditLayersKeeper.Instance().setState(GIEditingStatus.WAITIN_FOR_SELECT_OBJECT);
				m_btnNew.setEnabled(false);
				m_btnGeometry.setEnabled(false);
				m_btnDelete.setEnabled(false);
		...
		if(v.getId() == R.id.edit_layer_geometry)
		{
			...
			if((GIEditLayersKeeper.Instance().getState() == GIEditingStatus.EDITING_GEOMETRY)||(GIEditLayersKeeper.Instance().getState() == GIEditingStatus.WAITING_FOR_SELECT_GEOMETRY_TO_EDITING)||(GIEditLayersKeeper.Instance().getState() == GIEditingStatus.WAITING_FOR_NEW_POINT_LOCATION))
			{
				GIEditLayersKeeper.Instance().setState(GIEditingStatus.RUNNING);
				GIEditLayersKeeper.Instance().StopEditingGeometry();
				GIEditLayersKeeper.Instance().m_layer.m_Status = GIEditableLayerStatus.UNSAVED;
				m_btnNew.setEnabled(true);
			...
			{
				GIEditLayersKeeper.Instance().setState(GIEditingStatus.WAITING_FOR_SELECT_GEOMETRY_TO_EDITING);
